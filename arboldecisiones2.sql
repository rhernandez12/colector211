/*
SQLyog Community
MySQL - 10.0.34-MariaDB-0ubuntu0.16.04.1 : Database - arboldecisiones2
*********************************************************************
*/
...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`arboldecisiones2` /*!40100 DEFAULT CHARACTER SET latin1 */;

USE `arboldecisiones2`;
...
CREATE TABLE `averias7dias` (
  ...
  `fecreg` varchar(10) CHARACTER SET latin1 DEFAULT NULL,
...
CREATE TABLE `decisiones` (
  ...
  KEY `paso21` (`paso21`),
  KEY `paso22` (`paso22`)
) ENGINE=MyISAM AUTO_INCREMENT=2088 DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `paso05` (
  ...
  KEY `paso4` (`paso4`),
  KEY `imagen` (`img_total`)
) ENGINE=MyISAM AUTO_INCREMENT=297 DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `paso06` (
  ...
  KEY `paso5` (`paso5`),
  KEY `imagen` (`img_total`)
) ENGINE=MyISAM AUTO_INCREMENT=306 DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `paso12` (
  ...
  KEY `paso11` (`paso11`),
  KEY `imagen` (`img_total`)
) ENGINE=MyISAM AUTO_INCREMENT=185 DEFAULT CHARSET=utf8mb4;
...
BEGIN
	...
	DELETE FROM arboldecisiones.averias7dias WHERE DATEDIFF(NOW(),fecreg)>7;
	INSERT IGNORE arboldecisiones.averias7dias SELECT codcli,codreq,SUBSTR(fec_mov,1,16) AS fecreg FROM ccm1.`averias_m1_new` WHERE DATEDIFF(NOW(),fec_mov)<=7;
	INSERT IGNORE arboldecisiones.averias7dias SELECT codigodelcliente,codigoreq,SUBSTR(fecharegistro,1,16) AS fecreg FROM cms.`aver_liq_catv_pais` WHERE DATEDIFF(NOW(),fecharegistro)<=7;
	UPDATE arboldecisiones.reportearbol a INNER JOIN arboldecisiones.averias7dias b ON a.`idclientecrm`=b.`codli` SET a.`averia`=b.`codreq`,a.`fecreg`=b.`fecreg` WHERE TIMEDIFF(a.`fechahora`,b.`fecreg`)<='01:00:00.00000';
	UPDATE arboldecisiones.reportearbol a INNER JOIN cms.req_pend_macro_final b ON a.`idclientecrm`=b.`codcli` SET a.`averia`=b.`codreq`,a.`fecreg`=b.`fec_registro` WHERE TIMEDIFF(a.`fechahora`,b.`fec_registro`)<='01:00:00.00000';
	TRUNCATE TABLE arboldecisiones.reporte_final_arbol;
	INSERT INTO arboldecisiones.reporte_final_arbol
	SELECT SUBSTR(a.fechahora,1,10) AS FECHA,a.resultadoarbol AS RESULTADO_ARBOL,SUM(IF(averia=0,1,0)) AS SOL_EN_LINEA,SUM(IF(averia>0,1,0)) AS NO_SOL,COUNT(*) AS TOTAL,ROUND((SUM(IF(averia=0,1,0))/COUNT(*))*100,2) AS PORC  FROM `arboldecisiones`.`reportearbol`  a GROUP BY 1,2;
	SELECT 'FECHA','RESULTADO_ARBOL','SOL_EN_LINEA','NO_SOL','TOTAL','PORC'
	UNION
	SELECT * FROM arboldecisiones.reporte_final_arbol INTO OUTFILE '/temp/ReporteFinalArbol.csv' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\r\n';
...
BEGIN
	...
	FROM arboldecisiones2.paso00 a 
	LEFT JOIN arboldecisiones2.paso01 b
	ON a.id=b.paso0
	LEFT JOIN arboldecisiones2.paso02 c
	ON b.id=c.paso1
	LEFT JOIN arboldecisiones2.paso03 d
	ON c.id=d.paso2
	LEFT JOIN arboldecisiones2.paso04 e
	ON d.id=e.paso3
	LEFT JOIN arboldecisiones2.paso05 f
	ON e.id=f.paso4
	LEFT JOIN arboldecisiones2.paso06 g
	ON f.id=g.paso5
	LEFT JOIN arboldecisiones2.paso07 h
	ON g.id=h.paso6
	LEFT JOIN arboldecisiones2.paso08 i
	ON h.id=i.paso7
	LEFT JOIN arboldecisiones2.paso09 j
	ON i.id=j.paso8
	LEFT JOIN arboldecisiones2.paso10 k
	ON j.id=k.paso9
	LEFT JOIN arboldecisiones2.paso11 l
	ON k.id=l.paso10
	LEFT JOIN arboldecisiones2.paso12 m
	ON l.id=m.paso11
	LEFT JOIN arboldecisiones2.paso13 n
	ON m.id=n.paso12
	LEFT JOIN arboldecisiones2.paso14 o
	ON n.id=o.paso13
	LEFT JOIN arboldecisiones2.paso15 p
	ON o.id=p.paso14
	LEFT JOIN arboldecisiones2.paso16 q
	ON p.id=q.paso15
	LEFT JOIN arboldecisiones2.paso17 r
	ON q.id=r.paso16
	LEFT JOIN arboldecisiones2.paso18 s
	ON r.id=s.paso17
	LEFT JOIN arboldecisiones2.paso19 t
	ON s.id=t.paso18
	LEFT JOIN arboldecisiones2.paso20 u
	ON t.id=u.paso19
	LEFT JOIN arboldecisiones2.paso21 v
	ON u.id=v.paso20
	LEFT JOIN arboldecisiones2.paso22 w
	ON v.id=w.paso21;
    END */$$
DELIMITER ;
